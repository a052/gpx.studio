...
它本质上由一系列 GPS 点组成，用以编码形成一条或多条轨迹，以及标注点（可选）。
...
### <MapPin size="16" class="inline-block" style="margin-bottom: 2px" /> 标注点

**标注点**（waypoint）用来表示 GPS 设备或数字地图上需要显示的特殊位置。

除坐标外，标注点也可包含 **名称** 和 **描述** 信息。
