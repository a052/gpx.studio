---
title: Anstieg
---

<script>
    import { MountainSnow } from '@lucide/svelte';
    import Elevation from '$lib/components/toolbar/tools/Elevation.svelte';
    import DocsNote from '$lib/components/docs/DocsNote.svelte';
</script>

# <MountainSnow size="24" class="inline-block" style="margin-bottom: 5px" /> { title }

Mit diesem Tool kannst du Höhendaten zu Routen und [Wegpunkten](../gpx) hinzufügen oder die vorhandenen Daten ersetzen.

<div class="flex flex-row justify-center">
    <Elevation class="text-foreground p-3 border rounded-md shadow-lg" />
</div>

**Höhendaten löschen** entfernt alle Höhendaten aus der Auswahl, einschließlich der Höhen der Wegpunkte, wenn eine ganze Datei ausgewählt ist.

<DocsNote>

Elevation data is provided by <a href="https://mapterhorn.com" target="_blank">Mapterhorn</a>.
You can learn more about its origin and accuracy in the <a href="https://mapterhorn.com/attribution/" target="_blank">documentation</a>.

</DocsNote>
